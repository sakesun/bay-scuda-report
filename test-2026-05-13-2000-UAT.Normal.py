...
@app.function
def result_filter(f):
    return f.startswith("2026-05-13T19.5") or f.startswith("2026-05-13T20.0")
...
@app.cell
def _(alt, full_sources, mo):
    api_rows = mo.sql(f'{full_sources}')

    # Create an Altair chart
    _chart = alt.Chart(api_rows).mark_point().encode(
        x='timeStamp',
        y='elapsed'
